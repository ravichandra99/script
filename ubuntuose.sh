...
if [ -z "${REPOSITORY}" ]; then
  if [ "$WEB_UPGRADE" = false ]; then
    set -x
    REPOSITORY=${1:-https://github.com/ravichandra99/screenly-ose-test.git}
  else
    set -e
    REPOSITORY=https://github.com/ravichandra99/screenly-ose-test.git
  fi
fi
...
sudo -u ubuntu ansible localhost\
    -m git \
    -a "repo=$REPOSITORY dest=/home/ubuntu/screenly version=$BRANCH force=no"
cd /home/ubuntu/screenly/ansible

sudo -E ansible-playbook site.yml "${EXTRA_ARGS[@]}"

# Pull down and install containers
/home/ubuntu/screenly/bin/upgrade_containers.sh
...
if [ ! -f /etc/sudoers.d/010_ubuntu-nopasswd ]; then
  echo "ubuntu ALL=(ALL) NOPASSWD: ALL" | sudo tee /etc/sudoers.d/010_ubuntu-nopasswd > /dev/null
  sudo chmod 0440 /etc/sudoers.d/010_ubuntu-nopasswd
fi
...
check_defaultpw () {
    if [ "$BRANCH" = "master" ] || [ "$BRANCH" = "production" ] && [ "$WEB_UPGRADE" = false ]; then
        ...
        VAR_CURRENTPISALT=$(sudo cat /etc/shadow | grep ubuntu | awk -F '$' '{print $3}')
        VAR_CURRENTPIUSERPW=$(sudo cat /etc/shadow | grep ubuntu | awk -F ':' '{print $2}')
