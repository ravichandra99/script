...
if [ "$WEB_UPGRADE" = false ]; then
  ...
  cat << EOF
__   _____  ___ _ __ __ _  __ _  __ _ _ __ _   _ 
\ \ / / _ \/ _ \ '__/ _` |/ _` |/ _` | '__| | | |
 \ V /  __/  __/ | | (_| | (_| | (_| | |  | |_| |
  \_/ \___|\___|_|  \__,_|\__, |\__,_|_|   \__,_|
                          |___/                  
EOF
...
if [ -z "${REPOSITORY}" ]; then
  if [ "$WEB_UPGRADE" = false ]; then
    set -x
    REPOSITORY=${1:-https://github.com/ravichandra99/screenly-ose.git}
  else
    set -e
    REPOSITORY=https://github.com/ravichandra99/screenly-ose.git
  fi
fi
...
if [ "$BRANCH" = "master" ]; then
    ANSIBLE_VERSION=$(curl -s https://raw.githubusercontent.com/ravichandra99/screenly-ose/master/requirements/requirements.host.txt | grep ansible)
else
